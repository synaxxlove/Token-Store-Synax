    ...
    if id == owner_id:
        ...
        if "verify_" in message.text:
            ...
            if IS_VERIFY and not verify_status['is_verified']:
                short_url = f"vipurl.in"
                full_tut_url = f"https://t.me/Toutorialbysynax/4"
                token = ''.join(random.choices(string.ascii_letters + string.digits, k=10))
                await update_verify_status(id, verify_token=token, link="")
                link = await get_shortlink(SHORTLINK_URL, SHORTLINK_API,f'https://telegram.dog/{client.username}?start=verify_{token}')
                btn = [
                    [InlineKeyboardButton("Verify Now ☣️", url=link)],
                    [InlineKeyboardButton('How to use the bot 🌷', url=full_tut_url)]
                ]
                await message.reply(f"Your Ads token is expired, refresh your token and try again.\n\nToken Timeout: {get_exp_time(VERIFY_EXPIRE)}\n\nWhat is the token?\n\nThis is an ads token. If you pass 1 ad, you can use the bot for 12 Hour after passing the ad.\n\n (बस एक बार verify करने के बाद 12 घंटे तक Free मे वीडियो देखो बार बार लिंक खोले की कोई जरूरत नहीं है।)", reply_markup=InlineKeyboardMarkup(btn), protect_content=False, quote=True)
